...
function translate() {
  ...
  let textFile = new Blob([textContent], { type: 'text/plain' });

  // If we don't have a current file name, ask the user for one
  if (!currentFileName) {
  ...
  formData.append('file', textFile, currentFileName);
...
*/
  ...
  // let formData = new FormData();
  // formData.append('file', textarray, currentFileName);
...
//   var ctx = canvas.getContext('2d');
//   var title = extractedText; // replace this with user input
